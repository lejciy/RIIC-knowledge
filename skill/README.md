...
各 skill 由 84 题高频提问清单逐类实测知识库回答路径后提炼：链路完整但环节多、单次检索容易断链的问题形态，固化为下面的必走步骤；知识库单点即可答好的问题不设 skill。
...
- 新增 skill 前，先用高频问题实测知识库现有回答路径，确认断点真实存在再立项；先行立项的（如 skill-7），题号列待实测后补登。
